...
local function BuildBagsTab(scroll)
    ...
    -- BAG BAR VISIBILITY
    -- ====================================================================
    local visibilitySection = C:AddSection(scroll, LO["Bag Bar Visibility"])

    C:AddDescription(
        visibilitySection,
        LO["Show the bag bar only when selected conditions are active. The inventory and bank windows are not affected."]
    )

    C:AddToggle(visibilitySection, {
        label = LO["Show on Hover"],
        dbPath = "bags.visibility.show_on_hover",
        callback = function()
    ...
    C:AddToggle(visibilitySection, {
        ...
        dbPath = "bags.visibility.show_in_combat",
        callback = function()
            if addon.RefreshBagBarVisibility then
                addon.RefreshBagBarVisibility()
            end
    ...
    C:AddToggle(visibilitySection, {
        ...
        dbPath = "bags.visibility.show_with_target",
        callback = function()
            if addon.RefreshBagBarVisibility then
                addon.RefreshBagBarVisibility()
            end
    ...
    C:AddToggle(visibilitySection, {
        ...
        dbPath = "bags.visibility.show_on_health",
        callback = function()
            if addon.RefreshBagBarVisibility then
                addon.RefreshBagBarVisibility()
            end
    ...
    C:AddToggle(visibilitySection, {
        ...
        dbPath = "bags.visibility.show_on_power",
        callback = function()
            if addon.RefreshBagBarVisibility then
                addon.RefreshBagBarVisibility()
            end
    ...
    C:AddSlider(visibilitySection, {
        ...
        desc = LO["Time in seconds used to fade the bag bar in or out. Set to 0 for instant visibility changes."],
        dbPath = "bags.visibility.fade_duration",
        min = 0,
        max = 3,
        step = 0.05,
        width = 200,
