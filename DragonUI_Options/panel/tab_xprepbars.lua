...
local function BuildXpRepTab(scroll)
    ...
    C:AddToggle(textSection, {
        ...
        -- VISIBILITY
        -- ====================================================================
        local visibilitySection = C:AddSection(scroll, LO["Bar Visibility"])

        C:AddDescription(
            visibilitySection,
            LO["Show XP and reputation bars only when selected conditions are active. Existing XP and reputation availability rules still apply."]
        )

        C:AddHeading(visibilitySection, LO["Experience Bar"])

        C:AddToggle(visibilitySection, {
            label = LO["Show on Hover"],
            dbPath = "xprepbar.visibility.xp.show_on_hover",
            callback = RefreshBars,
        })

        C:AddToggle(visibilitySection, {
            label = LO["Show in Combat"],
            dbPath = "xprepbar.visibility.xp.show_in_combat",
            callback = RefreshBars,
        })

        C:AddToggle(visibilitySection, {
            label = LO["Show with Target"],
            dbPath = "xprepbar.visibility.xp.show_with_target",
            callback = RefreshBars,
        })

        C:AddToggle(visibilitySection, {
            label = LO["Show When Health Is Not Full"],
            dbPath = "xprepbar.visibility.xp.show_on_health",
            callback = RefreshBars,
        })

        C:AddToggle(visibilitySection, {
            label = LO["Show When Power Is Not Full"],
            dbPath = "xprepbar.visibility.xp.show_on_power",
            callback = RefreshBars,
        })

        C:AddSlider(visibilitySection, {
            label = LO["XP Bar Fade Duration"],
            dbPath = "xprepbar.visibility.xp.fade_duration",
            min = 0,
            max = 3,
            step = 0.05,
            width = 200,
        ...
        C:AddHeading(visibilitySection, LO["Reputation Bar"])

        C:AddToggle(visibilitySection, {
            label = LO["Show on Hover"],
            dbPath = "xprepbar.visibility.rep.show_on_hover",
            callback = RefreshBars,
        })

        C:AddToggle(visibilitySection, {
            label = LO["Show in Combat"],
            dbPath = "xprepbar.visibility.rep.show_in_combat",
            callback = RefreshBars,
        })

        C:AddToggle(visibilitySection, {
            label = LO["Show with Target"],
            dbPath = "xprepbar.visibility.rep.show_with_target",
            callback = RefreshBars,
        })

        C:AddToggle(visibilitySection, {
            label = LO["Show When Health Is Not Full"],
            dbPath = "xprepbar.visibility.rep.show_on_health",
            callback = RefreshBars,
        })

        C:AddToggle(visibilitySection, {
            label = LO["Show When Power Is Not Full"],
            dbPath = "xprepbar.visibility.rep.show_on_power",
            callback = RefreshBars,
        })

        C:AddSlider(visibilitySection, {
            label = LO["Reputation Bar Fade Duration"],
            dbPath = "xprepbar.visibility.rep.fade_duration",
            min = 0,
            max = 3,
            step = 0.05,
            width = 200,
