...
local function BuildMinimapTab(scroll)
    ...
    -- MINIMAP VISIBILITY
    -- ====================================================================
    local visibility = C:AddSection(scroll, LO["Minimap Visibility"])

    C:AddDescription(
        visibility,
        LO["Show the entire minimap, including the LFG eye and addon buttons, only when selected conditions are active."]
    )

    local function RefreshMinimapVisibility()
    ...
    C:AddToggle(visibility, {
        ...
        desc = LO["Reveal the entire minimap while the mouse is over its position."],
        dbPath = "minimap.visibility.show_on_hover",
        callback = RefreshMinimapVisibility,
    })

    C:AddToggle(visibility, {
        label = LO["Show in Combat"],
        desc = LO["Reveal the entire minimap while in combat."],
        dbPath = "minimap.visibility.show_in_combat",
        callback = RefreshMinimapVisibility,
    })

    C:AddToggle(visibility, {
        label = LO["Show with Target"],
        desc = LO["Reveal the entire minimap while a target exists."],
        dbPath = "minimap.visibility.show_with_target",
        callback = RefreshMinimapVisibility,
    })

    C:AddToggle(visibility, {
        label = LO["Show When Health Is Not Full"],
        desc = LO["Reveal the entire minimap while your health is below maximum."],
        dbPath = "minimap.visibility.show_on_health",
        callback = RefreshMinimapVisibility,
    })

    C:AddToggle(visibility, {
        label = LO["Show When Power Is Not Full"],
        desc = LO["Reveal the entire minimap while your mana or power is below maximum."],
        dbPath = "minimap.visibility.show_on_power",
        callback = RefreshMinimapVisibility,
    })
    ...
    C:AddSlider(visibility, {
        ...
        dbPath = "minimap.visibility.fade_duration",
        min = 0,
        max = 3,
        step = 0.05,
        width = 200,
